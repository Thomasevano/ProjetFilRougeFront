...
const TreeMap = ({ monuments }) => {
  ...
  React.useEffect(() => {
    ...
    d3.selectAll(".trimobiles")
      ...
      .append("div")
      .attr("class", "goThere goThereTrimobiles");

    d3.selectAll(".goThereTrimobiles")
      .append("xhtml:img")
      .attr("src", "./data/treeMap/goThere.svg");

    d3.selectAll(".goThereTrimobiles").append("xhtml:p").text("Go There");
  ...
  React.useEffect(() => {
    ...
    d3.selectAll(".trilibs")
      ...
      .append("div")
      .attr("class", "goThere goThereTrilibs");

    d3.selectAll(".goThereTrilibs")
      .append("xhtml:img")
      .attr("src", "./data/treeMap/goThere.svg");

    d3.selectAll(".goThereTrilibs").append("xhtml:p").text("Go There");
  ...
  const drawChart = (listMonument) => {
    ...
    d3.treemap().size([width, height]).padding(20)(root);
    ...
    vignette
      ...
      .style("height", function (d) {
        ...
        var p2Size = document.querySelector(
          ".vignette" + d.data.id + " .sport"
        );
        var sizeVignette = document.querySelector(
          ".vignette" + d.data.id + " div"
        );
        var vSize = d.y1 - d.y0;
    ...
    const backMondrian = zoomVignette
      .append("div")
      .attr("class", "buttonBack")
      .on("click", handleClickBackMondrian);

    backMondrian.append("img").attr("src", "./data/arrowSecondaryButton.svg");

    backMondrian.append("p").text("Back to Mondrian Grid");

    const illustration = zoomVignette.append("div").attr("class", "imageZoom");

    illustration.append("h3").text((d) => d.name);

    illustration.append("h4").text((d) => d.sport);

    illustration.append("img").attr("src", (d) => d.img_url);

    const contentVignette = zoomVignette
      .append("div")
    ...
    button
      ...
      .text(
        "Tri Mobile is a device that allows you to drop off all of your bulky items."
      );
    ...
    button
      ...
      .text(
        "These proximity waste containers allows you to collect more and better, in particular glass, plastic and metal packaging, paper and cardboard."
      );

    function handleClickMonument() {
      d3.select(".vignetteMonumentContainer").style("display", "block");

      d3.select(".treeMap svg").style("display", "none");

      //choose the vignette to display
      var vignette = document.querySelector(
        ".zoomMonument." + this.classList[1]
      );

      if (window.innerWidth > 1024) {
        d3.select(vignette).style("display", "flex");
      } else {
    ...
    function handleClickButton() {
      ...
      if (
        ...
        this.classList.contains("buttonTrimobile")
      ) {
        for (let i = 0; i < trilib.length; i++) {
          trilib[i].classList.remove("active");
          trimobile[i].classList.add("active");

          d3.selectAll(".vignetteTrimobiles").style("display", "block");

          d3.selectAll(".vignetteTrilibs").style("display", "none");

          d3.selectAll(".textTrimobile").style("display", "block");

          d3.selectAll(".textTrilib").style("display", "none");

          d3.selectAll(".imageTrimobile").style("display", "inline");

          d3.selectAll(".imageTrilib").style("display", "none");
        }
      } else if (
        !this.classList.contains("active") &&
        this.classList.contains("buttonTrilib")
      ) {
        for (let i = 0; i < trilib.length; i++) {
          trimobile[i].classList.remove("active");
          trilib[i].classList.add("active");

          d3.selectAll(".vignetteTrimobiles").style("display", "none");

          d3.selectAll(".vignetteTrilibs").style("display", "block");

          d3.selectAll(".textTrimobile").style("display", "none");

          d3.selectAll(".textTrilib").style("display", "block");

          d3.selectAll(".imageTrimobile").style("display", "none");

          d3.selectAll(".imageTrilib").style("display", "inline");
        }
      }
    }
    ...
    function handleClickBackMondrian() {
      ...
      d3.select(".treeMap svg").style("display", "block");

      d3.select(this.parentNode).style("display", "none");

      d3.select(".vignetteMonumentContainer").style("display", "none");
    }
  };
